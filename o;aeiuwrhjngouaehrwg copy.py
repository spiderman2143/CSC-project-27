...
def init_db():
    ...
    cur = con.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS login (fullname VARCHAR(30) NOT NULL, age INT, gender VARCHAR(10), email VARCHAR(50), username VARCHAR(50) PRIMARY KEY, password VARCHAR(20), bio VARCHAR(50))")
    con.commit()
    con.close()
...
def user_signup():
    ...
    try:
        ...
        cur = con.cursor()
        cur.execute(
            "INSERT INTO login (fullname, age, gender, email, username, password, bio) VALUES (%s,%s,%s,%s,%s,%s,%s)",
